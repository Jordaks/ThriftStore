<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if(!empty($email) && !empty($encpass)) {
            ...
            $query = "SELECT * FROM thriftstore_db WHERE email = '$email' limit 1";  
            ...
            if($result){
                if($result && mysqli_num_rows($result) > 0){
                    ...
                    if ($user_data['password'] === $encpass) {
                        ...
                        $_SESSION["house_number"] = $user_data["house_Number"];
                        $_SESSION["zone"] = $user_data["zone"];
    ...
    <div class="bg-[url('/ThriftStore/src/image/backg.jpg')] bg-center bg-cover px-6 min-h-screen grid place-items-center">
        <div class="bg-white p-11   rounded-3xl shadow-2xl w-1/2 ">
            ...
            <div class="mt-12 text-sm font-semibold text-center">
                Don't have an account ? <a href="signup.php" class="text-pink-400 hover:text-pink-700">Sign Up</a>
            </div>
        </div>
    </div>
